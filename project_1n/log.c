...
void print_log(int file) {
    cnt = count_lines_in_file("/home/viratgarg/Documents/Sem3/OSN/projects/project_1n/newfile.txt");
...
void extract_last_line(const char *filename, char *buffer, size_t buffer_size) {
    ...
    while (pos >= 0) {
        ...
        if (ch == '\n') {
            if (line_count == 1) {
                pos++;  // Move forward to start of the actual last line
                break;
            }
            line_count++;
        }
    ...
    // If we reached the beginning of the file without finding a newline
    if (pos < 0) {
        pos = 0; // Start reading from the beginning of the file
    }
